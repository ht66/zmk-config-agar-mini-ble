#define DT_DRV_COMPAT zmk_behavior_two_axis_move
...
struct behavior_two_axis_move_config {
    int16_t x_code, y_code;
    uint16_t delay_ms, time_to_max_speed_ms;
    uint8_t trigger_period_ms, acceleration_exponent;
};
...
static float speed(const struct behavior_two_axis_move_config *cfg, float max_speed,
...
static float update_movement_1d(const struct behavior_two_axis_move_config *cfg,
...
static struct vector2d update_movement_2d(const struct behavior_two_axis_move_config *cfg,
...
// ---- 双速状态（极简） ----
struct behavior_two_axis_move_data {
    struct k_work_delayable tick_work;
    const struct device *dev;
    struct movement_state_2d state;
    int16_t quick_sum_x, quick_sum_y;
    int16_t slow_sum_x, slow_sum_y;
    uint8_t active_slot; // 0：快速，1：慢速
};

static void adjust_speed(const struct device *dev, int16_t dx, int16_t dy) {
    struct behavior_two_axis_move_data *data = dev->data;
...
static void update_work_scheduling(const struct device *dev) {
    struct behavior_two_axis_move_data *data = dev->data;
    const struct behavior_two_axis_move_config *cfg = dev->config;
...
static void tick_work_cb(struct k_work *work) {
    ...
    struct behavior_two_axis_move_data *data =
    CONTAINER_OF(dwork, struct behavior_two_axis_move_data, tick_work);
...
// 公开接口
void behavior_two_axis_set_active_slot(const struct device *dev, uint8_t slot) {
    struct behavior_two_axis_move_data *data = dev->data;
...
static int behavior_two_axis_move_init(const struct device *dev) {
    struct behavior_two_axis_move_data *data = dev->data;
    data->dev = dev;
    data->active_slot = 0;
...
#define TWO_AXIS_MOVE_INST(n)                                                                      \
    static struct behavior_two_axis_move_data two_axis_move_data_##n;                              \
    static const struct behavior_two_axis_move_config two_axis_move_config_##n = {                 \
    ...
    BEHAVIOR_DT_INST_DEFINE(n, behavior_two_axis_move_init, NULL, &two_axis_move_data_##n,        \
                            &two_axis_move_config_##n, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, \
                            &api);

DT_INST_FOREACH_STATUS_OKAY(TWO_AXIS_MOVE_INST)